...
!!! warning "Most control types are not implemented yet"
    ...
    **SwitchMultiPos** + **AnalogMultiPos** + **Switch3Pos** (inputs), plus the **PinRef** abstraction. LED / Switch2Pos / PinRef are
    ...
    NeedleGauge and Switch3Pos are authored and compile-gated, with the on-hardware bench still pending; SwitchMultiPos and AnalogMultiPos are hardware-verified.
...
| `Switch3Pos` | **Implemented** (bench pending) | 3-position (ON-OFF-ON). value 0/1/2 |
