...
!!! warning "Most control types are not implemented yet"
    ...
    **SwitchMultiPos** + **AnalogMultiPos** + **AnalogInput** (inputs), plus the **PinRef** abstraction. LED / Switch2Pos / PinRef are
    ...
    NeedleGauge is authored and compile-gated, with the on-hardware bench still pending; SwitchMultiPos, AnalogMultiPos, and AnalogInput are hardware-verified.
