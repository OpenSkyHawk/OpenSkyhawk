...
!!! warning "Most control types are not implemented yet"
    ...
    **SwitchMultiPos** + **AnalogMultiPos** + **AnalogInput** (inputs), plus the **PinRef** abstraction. LED / Switch2Pos / PinRef are
    hardware-verified (Phase 3); DrumDisplay is hardware-verified (mux + readouts on real OLEDs).
    NeedleGauge and AnalogInput are authored and compile-gated, with the on-hardware bench still pending; SwitchMultiPos and AnalogMultiPos are hardware-verified.
...
| `AnalogInput` | **Implemented** (bench pending) | Continuous analog, normalised to 16-bit (EWMA + hysteresis) |
