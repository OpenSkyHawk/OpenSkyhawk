// RotaryAcceleratedEncoder — momentum_reset test (#287)
//
// Momentum only protects a spin in progress. Reversing straight after a CW detent is eaten by the
// filter (four transitions just decay the momentum to zero); after STOPPED_THRESHOLD_MS with no
// detent the momentum resets, so a deliberate reversal from rest registers in full.
//
// Rig: this STM32 on the CAN bus with the PanelBridge (node ACKs). No encoder hardware needed.
#include <Arduino.h>
#include <STM32Board.h>
#include <Inputs/RotaryAcceleratedEncoder/RotaryAcceleratedEncoder.h>

namespace OS = OpenSkyhawk;
static constexpr uint16_t CTRL_ID = 0x567B;
static bool gPass = true;
static void check(const char* label, bool ok) {
    if (!ok) gPass = false;
    STM32Board::diagSerial().print(label);
    STM32Board::diagSerial().println(ok ? ": PASS" : ": FAIL");
}
// One full detent at EncoderStepsPerDetent::Four, starting and ending at Gray state 0.
template <class E> static void cw(E& e)  { e.debugStep(1); e.debugStep(3); e.debugStep(2); e.debugStep(0); }
template <class E> static void ccw(E& e) { e.debugStep(2); e.debugStep(3); e.debugStep(1); e.debugStep(0); }

OS::RotaryAcceleratedEncoder gEnc(CTRL_ID, PinRef(PA0), PinRef(PA1), OS::EncoderStepsPerDetent::Four,
                                  /*step*/ 3200, /*fastStep*/ 0);

void setup() {
    STM32Board::setDebug(true);
    STM32Board::begin();
    STM32Board::diagSerial().println("=== RotaryAcceleratedEncoder momentum_reset ===");
    gEnc.configure();
    CANProtocol::start();
    gEnc.debugSeed(0);

    cw(gEnc);                                                   // +1, momentum 4
    ccw(gEnc);                                                  // all four transitions dropped
    check("immediate reversal eaten by momentum", gEnc.emitCount() == 1 && gEnc.momentum() == 0);

    cw(gEnc);                                                   // +1 again, momentum 4
    delay(OS::RotaryEncoder::STOPPED_THRESHOLD_MS + 100);
    gEnc.debugStep(0);                                          // idle sample → momentum reset
    check("momentum reset after the stop threshold", gEnc.momentum() == 0);
    ccw(gEnc);
    check("reversal from rest registers", gEnc.emitCount() == 3 && gEnc.lastValue() == -3200);

    CANProtocol::flushBatched(canIdEvtRel(NODE_ID));
    STM32Board::diagSerial().println(gPass ? "=== ALL PASS ===" : "=== FAIL ===");
}
void loop() {}
