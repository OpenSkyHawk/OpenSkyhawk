...
//
// Spinning quickly through intermediate detents (each held < DEBOUNCE_MS) emits ONLY the settled
// position — the debounce filters the transients. Verified via position()/emitCount(); CAN in
...
void setup() {
    ...
    check("mid-sweep: nothing confirmed yet", gSel.emitCount() == before);

    // Settle on 3.
    delay(25);
    gSel.poll(); txPush();
    check("settled: single EVT", gSel.emitCount() == before + 1);
    check("settled: value is the final index 3", gSel.position() == 3);
